...
# --- Dar acceso al informe exige que haya versión FINAL publicada (un borrador no cuenta) ---


def test_dar_acceso_bloqueado_sin_informe_final(client, as_session, user_session, monkeypatch):
    """Sin versión final el advisee vería un informe vacío: el toggle no debe llegar a Notion."""
    llamadas = []
    monkeypatch.setattr(ca_router, "existe_informe_final", lambda advisee: False)
    monkeypatch.setattr(ca_router, "toggle_acceso_advisee_individual", lambda *a, **k: llamadas.append(a))
    monkeypatch.setattr(ca_router, "notificar_acceso_informe_final_web", lambda *a, **k: llamadas.append(a))
    as_session(user_session)
    r = client.post("/api/acceso-advisee-individual", json={"advisee": "Juan Perez", "activo": True})
    assert r.status_code == 400
    assert "borrador" in r.json()["error"].lower()
    assert llamadas == []  # ni se toca el acceso ni se avisa por Slack


def test_dar_acceso_permitido_con_informe_final(client, as_session, user_session, monkeypatch):
    avisados = []
    monkeypatch.setattr(ca_router, "existe_informe_final", lambda advisee: True)
    monkeypatch.setattr(ca_router, "toggle_acceso_advisee_individual", lambda *a, **k: True)
    monkeypatch.setattr(ca_router, "notificar_acceso_informe_final_web", lambda a: avisados.append(a))
    as_session(user_session)
    r = client.post("/api/acceso-advisee-individual", json={"advisee": "Juan Perez", "activo": True})
    assert r.status_code == 200
    assert r.json() == {"ok": True, "activo": True}
    assert avisados == ["Juan Perez"]


def test_revocar_acceso_permitido_aunque_no_haya_informe_final(client, as_session, user_session, monkeypatch):
    """Quitar un acceso ya dado nunca debe quedar bloqueado por la comprobación."""
    monkeypatch.setattr(ca_router, "existe_informe_final", lambda advisee: False)
    monkeypatch.setattr(ca_router, "toggle_acceso_advisee_individual", lambda *a, **k: True)
    as_session(user_session)
    r = client.post("/api/acceso-advisee-individual", json={"advisee": "Juan Perez", "activo": False})
    assert r.status_code == 200
    assert r.json() == {"ok": True, "activo": False}


def test_acceso_individual_get_informa_de_si_hay_informe_final(client, as_session, user_session, monkeypatch):
    """El front necesita el sí/no para deshabilitar el botón antes de que lo pulsen."""
    monkeypatch.setattr(ca_router, "advisee_tiene_acceso_individual", lambda *a, **k: False)
    monkeypatch.setattr(ca_router, "existe_informe_final", lambda advisee: False)
    as_session(user_session)
    r = client.get("/api/acceso-advisee-individual", params={"advisee": "Juan Perez"})
    assert r.status_code == 200
    assert r.json() == {"activo": False, "informeFinal": False}
