...
def _solo_top_down(evaluaciones: list[dict]) -> list[dict]:
    """Evaluaciones mensuales que el CA puede leer en bruto: solo las de un superior.

    Más estricto que excluir_feedback_confidencial (que solo quita bottom-to-top): aquí
    caen también las de iguales y las de dirección desconocida. El motivo es el tamaño del
    equipo — sabiendo que una evaluación viene de un peer, el advisee tiene tres o cuatro
    candidatos, así que quitar el nombre no la anonimiza. Sin dirección registrada no se
    puede descartar que fuera de un subordinado, y ante la duda no se publica.
    El feedback bottom-to-top sigue accesible solo desde el panel de administración.
    """
    return [e for e in evaluaciones if e.get("relacion") == "superior"]
...
# Las entradas de evals de proyecto y mensuales se construyen aquí y en ningún otro sitio:
# el PDF individual y el combinado las montaban por separado y se desincronizaron (el
...
def _entradas_evals_mensuales(advisee, anonimo, idioma="es"):
    """Evaluaciones mensuales: solo las de un superior (ver _solo_top_down) y sin nombre
    ni etiqueta de nivel."""
    datos = sorted(_solo_top_down(obtener_evaluaciones_por_evaluado(advisee)), key=lambda x: x.get("fecha", ""))
    out = []
    for d in datos:
        cuerpo = []
        if d.get("q1"):
...
def generar_pdf_evals_proyecto(advisee: str, anonimo: bool = True, idioma: str = "es") -> str:
    entradas = _entradas_evals_proyecto(advisee, anonimo, idioma)
...
def generar_pdf_evals_mensuales(advisee: str, anonimo: bool = True, idioma: str = "es") -> str:
    entradas = _entradas_evals_mensuales(advisee, anonimo, idioma)
    slug = slug_archivo(advisee)
...
# Las evals de proyecto y mensuales reutilizan _entradas_evals_proyecto/_mensuales de
# arriba, así que las mismas reglas de privacidad aplican aquí sin repetirlas.
...
def _entradas_opiniones(advisee, ca, anonimo=True, idioma="es"):
    datos = sorted(obtener_opiniones_ca_por_advisee(ca, advisee), key=lambda x: x.get("fecha", ""))
    out = []
    for d in datos:
        cuerpo = []
        if d.get("opinion"):
...
def _construir_pdf_secciones(titulo, advisee, ca, secciones, nombre_archivo, idioma="es"):
    ...
    for sec_tit, entradas in secciones:
        if entradas is None:
            story.append(Paragraph(_esc(sec_tit), s_sec))
            story.append(Paragraph(_t(idioma, "fuente_no_disponible"), s_cuerpo))
            continue
...
def _fuente(titulo, fn):
    """Aísla una fuente del PDF combinado: si falla, esa sección queda marcada como
    no disponible (entradas=None) y el resto del informe se genera igualmente.
    None (fallo) y [] (leída, sin datos) se muestran distinto: un informe de RRHH no
    puede dar por vacía una fuente que en realidad no se pudo leer."""
    try:
        return (titulo, fn())
    except Exception:
        logging.exception("PDF completo: no se pudo leer la fuente '%s'", titulo)
        return (titulo, None)
...
def generar_pdf_completo(advisee: str, anonimo: bool = True, idioma: str = "es") -> str:
    ...
    ca = _ca_de(advisee)
    secciones = [
        _fuente(_t(idioma, "s_opiniones"), lambda: _entradas_opiniones(advisee, ca, anonimo=anonimo, idioma=idioma)),
        _fuente(_t(idioma, "t_mensuales"), lambda: _entradas_evals_mensuales(advisee, anonimo, idioma)),
        _fuente(_t(idioma, "t_proyecto"), lambda: _entradas_evals_proyecto(advisee, anonimo=anonimo, idioma=idioma)),
        _fuente(_t(idioma, "t_seguimiento"), lambda: _entradas_seguimiento(advisee, anonimo, idioma)),
        _fuente(_t(idioma, "t_evals_extra"), lambda: _entradas_evals_extra(advisee, anonimo=False, idioma=idioma)),
    ]
    slug = slug_archivo(advisee)
